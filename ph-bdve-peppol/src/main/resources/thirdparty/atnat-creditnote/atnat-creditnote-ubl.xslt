...
<!--SCHEMA SETUP-->
<xsl:template match="/">
    <svrl:schematron-output schemaVersion="" title="atnat-creditnote T14 bound to UBL">
      <xsl:comment>
...
<!--SCHEMATRON PATTERNS-->
<svrl:text>atnat-creditnote T14 bound to UBL</svrl:text>

<!--PATTERN UBL-T14-->


	<!--RULE -->
<xsl:template match="/ubl:CreditNote/cac:AllowanceCharge" mode="M5" priority="1003">
...
<xsl:choose>
	...
	<!--RULE -->
<xsl:template match="/ubl:CreditNote/cac:AdditionalDocumentReference" mode="M5" priority="1002">
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="count(cac:Attachment/cac:ExternalReference) = 0">
          <xsl:attribute name="id">ATNAT-T14-R008</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute>
          <svrl:text>[ATNAT-T14-R008]-Attachments to the invoice must be included into the invoice document and may not be referenced from external sources.</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
    <xsl:apply-templates mode="M5" select="*|comment()|processing-instruction()" />
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="(cac:Delivery/cbc:ActualDeliveryDate) or (cac:InvoicePeriod/cbc:StartDate and cac:InvoicePeriod/cbc:EndDate)">
          ...
          <svrl:text>[ATNAT-T14-R003]-The credit note MUST contain either the actual delivery date or the invoice period.</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
